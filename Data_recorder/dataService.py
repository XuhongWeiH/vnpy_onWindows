...
def downDailyBarBySymbol(symbol, info, end_date):
    ...
    # print(u'合约%s日线数据下载完成，耗时%.5s秒' % (symbol, cost_time))
...
#----------------------------------------------------------------------
# 按日期一次性补全数据
def downloadBarByDate(start_date=None, end_date=datetime.today().date()):
    jqdatasdk.auth(JQDATA_USER, JQDATA_PASSWORD)
    """下载所有配置中的合约的分钟线数据"""
    print('-' * 50)
    print(u'开始下载合约分钟线数据')
    ...
    for index, row in symbols_df.iterrows():
        ...
        print(u'合约%s日线数据下载完成，耗时%.5s秒' % (row['display_name'], cost_time))
        # break
