...
def show_chart(df,k):
    fig = make_subplots(rows=4, cols=1, subplot_titles=["Balance:" + k, "Drawdown", "Daily Pnl", "Pnl Distribution"], vertical_spacing=0.06)
...
for k in information_future.keys():
    ...
    with open("PARA/future_op_para_refine/" + k, encoding='utf-8') as fr:
    ...
    # show_chart(df, k)
    engine.show_chart(k=k, rf='D:/vnpy-2.1.2/vnpy-2.1.2/PARA/RES_refine/' + k + '.png')
    sleep(1)
